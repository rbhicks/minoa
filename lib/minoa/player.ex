defmodule Minoa.Player do
  ...
  def handle_call(
        :place_player_randomly,
        _from,
        %{pid: pid, position: {previous_x, previous_y}} = state
      ) do
    {x, y} = GenServer.call(:maze_server, :get_random_open_square)
  ...
  def handle_call(
    ...
    if(
      GenServer.call(
        :maze_server,
        {:closed_square?, {previous_x - 1, previous_y}}
      )
    ) do
      ...
      {:reply,
       GenServer.call(
         :maze_server,
         {:update_player_position, {{previous_x, previous_y}, {previous_x - 1, previous_y}, pid}}
  ...
  def handle_call(
    ...
    if(
      GenServer.call(
        :maze_server,
        {:closed_square?, {previous_x, previous_y + 1}}
      )
    ) do
      {:reply, {previous_x, previous_y}, state}
    else
      {:reply,
       GenServer.call(
         :maze_server,
         {:update_player_position, {{previous_x, previous_y}, {previous_x, previous_y + 1}, pid}}
